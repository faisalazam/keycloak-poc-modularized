...
sed -i "s#{{PGADMIN_SERVER_HOST}}#${KEYCLOAK_DB_HOST_NAME}#g" "$PGPASS_PATH"
sed -i "s#{{PGADMIN_SERVER_PORT}}#${POSTGRES_INTERNAL_PORT}#g" "$PGPASS_PATH"
sed -i "s#{{PGADMIN_SERVER_USER}}#${POSTGRES_USER}#g" "$PGPASS_PATH"
sed -i "s#{{PGADMIN_SERVER_PASSWORD}}#${POSTGRES_PASSWORD}#g" "$PGPASS_PATH"
